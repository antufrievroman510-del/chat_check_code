...
void Overlay::LoadConfig(Aimbot* aim) {
    ...
    handlers["aim_enable"] = [&](const std::string& v) { this->aim_enable = std::stoi(v); };
    ...
    handlers["shadow_webhook"] = [&](const std::string& v) { strncpy_s(shadow_webhook, sizeof(shadow_webhook), v.c_str(), 255); };
    ...
    handlers["aim_target"] = [&](const std::string& v) { aim_target = std::stoi(v); };
    handlers["rcs_enable"] = [&](const std::string& v) { rcs_enable = std::stoi(v); };
    handlers["rcs_pitch"] = [&](const std::string& v) { rcs_pitch = safe_stof(v); };
    handlers["rcs_yaw"] = [&](const std::string& v) { rcs_yaw = safe_stof(v); };
    handlers["elite_ballistics_enabled"] = [&](const std::string& v) { elite_ballistics_enabled = std::stoi(v); };
    ...
    handlers["spoofer_vid"] = [&](const std::string& v) { strncpy_s(spoofer_vid, sizeof(spoofer_vid), v.c_str(), 4); };
    handlers["spoofer_pid"] = [&](const std::string& v) { strncpy_s(spoofer_pid, sizeof(spoofer_pid), v.c_str(), 4); };
        ...
        if (!connected) {
            ...
            if (ImGui::Button("Apply/Connect", ImVec2(150, 35))) {
                ...
                if (result) {
                    ...
                    this->apply_hw_flag = true;
                    // Также синхронизируем настройки
